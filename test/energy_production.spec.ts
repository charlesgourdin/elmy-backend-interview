...
describe('Energy Production Service', () => {
    ...
    // it(`it should set all power plants formated json to the same interval`, () => {
    //     const data = [
    //         {
    //             start: 0,
    //             end: 10,
    ...
    it(`it should aggregate all Api responses into one`, () => {
        const data = [
            [
                {
                    start: 0,
        ...
        const expectedResultSum = data.map(powerPlant => {
            return powerPlant.reduce((acc, { power }) => {
                return acc + power
            }, 0)
        }).reduce((acc, value) => {
            return acc + value
        ...
        const result = EnergyProductionService.aggregateAllResult(data)
        const resultSum = result.reduce((acc, { power }) => {
            return acc + power
        }, 0)
